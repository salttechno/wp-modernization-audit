...
<body>
  <div class="container">
    ...
    <div class="section">
      ...
      ${renderCoreWebVitalsSection(result)}
    </div>
    ...
    <footer>
      <p>Generated by <a href="https://github.com/salttechno/wp-modernization-audit" target="_blank">wp-modernization-audit</a> v0.4.0</p>
...
function renderCoreWebVitalsSection(result: AuditResult): string {
  ...
  const getVitalIcon = (status: "good" | "needs-improvement" | "poor") => {
    switch (status) {
      case "good":
        return "✅";
      case "needs-improvement":
  ...
  return `
    <div style="margin-top: 30px; padding: 25px; background: #f0f9ff; border-radius: 8px; border-left: 4px solid #667eea;">
      <h3 style="color: #1f2937; margin-bottom: 15px;">🚀 Core Web Vitals (via PageSpeed Insights API)</h3>
      <table style="width: 100%; border-collapse: collapse; background: white; border-radius: 6px; overflow: hidden;">
        <thead>
          <tr style="background: #f3f4f6;">
            <th style="padding: 12px; text-align: left; border-bottom: 2px solid #e5e7eb;">Metric</th>
            <th style="padding: 12px; text-align: center; border-bottom: 2px solid #e5e7eb;">Value</th>
        ...
        <tbody>
          <tr style="border-bottom: 1px solid #f3f4f6;">
            <td style="padding: 12px; font-weight: 500;">LCP (Largest Contentful Paint)</td>
            <td style="padding: 12px; text-align: center;">${(
              homepageLighthouseData.lcp / 1000
            ).toFixed(2)}s</td>
            <td style="padding: 12px; text-align: center;">
              <span style="color: ${getVitalColor(
                vitals.lcpStatus
              )}; font-weight: 600;">${getVitalIcon(
    vitals.lcpStatus
  )} ${vitals.lcpStatus.replace("-", " ").toUpperCase()}</span>
            </td>
            <td style="padding: 12px; text-align: center; color: #6b7280;">&lt; 2.5s</td>
          </tr>
          <tr style="border-bottom: 1px solid #f3f4f6;">
            <td style="padding: 12px; font-weight: 500;">CLS (Cumulative Layout Shift)</td>
            <td style="padding: 12px; text-align: center;">${homepageLighthouseData.cls.toFixed(
              3
            )}</td>
            <td style="padding: 12px; text-align: center;">
              <span style="color: ${getVitalColor(
    ...
    vitals.clsStatus
  )} ${vitals.clsStatus.replace("-", " ").toUpperCase()}</span>
            </td>
            <td style="padding: 12px; text-align: center; color: #6b7280;">&lt; 0.1</td>
          </tr>
          <tr style="border-bottom: 1px solid #f3f4f6;">
            <td style="padding: 12px; font-weight: 500;">INP (Interaction to Next Paint)</td>
            <td style="padding: 12px; text-align: center;">${
    ...
    vitals.inpStatus
  )} ${vitals.inpStatus.replace("-", " ").toUpperCase()}</span>
            </td>
            <td style="padding: 12px; text-align: center; color: #6b7280;">&lt; 200ms</td>
          </tr>
          <tr>
            <td style="padding: 12px; font-weight: 500;">TTFB (Time to First Byte)</td>
            <td style="padding: 12px; text-align: center;">${
    ...
    vitals.ttfbStatus
  )} ${vitals.ttfbStatus.replace("-", " ").toUpperCase()}</span>
            </td>
            <td style="padding: 12px; text-align: center; color: #6b7280;">&lt; 800ms</td>
          </tr>
        </tbody>
      </table>
      <div style="margin-top: 15px; padding: 15px; background: white; border-radius: 6px;">
        <p style="margin: 5px 0;"><strong>Lighthouse Performance Score:</strong> ${Math.round(
          homepageLighthouseData.performanceScore
        )}/100</p>
        <p style="margin: 5px 0;"><strong>Strategy:</strong> ${
          homepageLighthouseData.strategy.charAt(0).toUpperCase() +
          homepageLighthouseData.strategy.slice(1)
        }</p>
        <p style="margin: 5px 0;"><strong>Data Source:</strong> Google PageSpeed Insights API 
          (<a href="https://pagespeed.web.dev/analysis?url=${encodeURIComponent(
            result.url
          )}" target="_blank" style="color: #667eea;">View full report</a>)
        </p>
      </div>
    </div>
  `;
